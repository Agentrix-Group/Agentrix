...
func TestIntegration_Engine_ReproducibleExecutionWithSpec(t *testing.T) {
	...
	r.Equal("agentrix-engine/1", spec.ProtocolVersion)
	r.Equal("starfighter", spec.GameID)
	r.Equal(int64(42), spec.Seed)
	r.Equal(manifest.MaxTicks, spec.Limits.MaxTicks)
	r.Equal(engineDigest, spec.EngineDigest)
	r.NotEmpty(spec.ConfigHash)
	r.NotEmpty(spec.SpecHash)
	r.Len(spec.Slots, 2)
	r.Equal("sub-alice-01", spec.Slots[0].SubmissionID)
	r.Equal("sub-bob-01", spec.Slots[1].SubmissionID)
